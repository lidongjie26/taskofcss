...
function initAqiChartData() {
    ...
    for (var key in aqiSourceData) {
        var tempCity = aqiSourceData[key];//获得每个数据
        var keyArr = Object.getOwnPropertyNames(tempCity);//所有日期组成的数组
        var tempMonth = keyArr[0].slice(5, 7);//月份
        var weekInit = 4, weekCount = 0;
        //循环每一天
        for (var i = 0; i < keyArr.length; i++, weekInit++) {
            count += tempCity[keyArr[i]];//所有空气质量的和
